...
% Notas al pie numeradas: la clase las define con símbolos (*, †, ...)
...
\ifnum\revision>0
1      & Se completa hasta la sección 5 inclusive               & 5 de septiembre de 2026 \\ \hline
\fi
\ifnum\revision>1
2      & Se completa hasta la sección 9 inclusive               & 11 de septiembre de 2026 \\ \hline
\fi
\ifnum\revision>2
3      & Se completa hasta la sección 12 inclusive              & 17 de septiembre de 2026 \\ \hline
\fi
\ifnum\revision>3
4      & Se completa el plan	                                & 21 de septiembre de 2026 \\ \hline
\fi
\ifnum\revision>3
5      & Se aplican correcciones finales                        & 23 de septiembre de 2026 \\ \hline
\fi
...
Por medio de la presente se acuerda con el \authorname\hspace{1px} que su Trabajo Final de la \degreename\hspace{1px} se titulará ``\ttitle'' y consistirá en el desarrollo de un modelo de aprendizaje profundo de bajo consumo de memoria y de un prototipo de firmware que ejecutará su inferencia sobre un SoC (del inglés \textit{system on chip}) basado en ARM Cortex-M33. El modelo clasificará la respuesta al impulso del canal (CIR, del inglés \textit{channel impulse response}) de un enlace de banda ultraancha (UWB, del inglés \textit{ultra-wideband}) según exista o no línea de visión entre los dispositivos (LoS y NLoS, del inglés \textit{line-of-sight} y \textit{non-line-of-sight}). El trabajo tendrá un presupuesto preliminar estimado de 600 horas y un costo estimado de USD 15.000, con fecha de inicio el \fechaInicioName\hspace{1px} y fecha de presentación pública el \fechaFinalName.
...
Los transceptores UWB permiten medir distancias con una precisión del orden del centímetro y sostienen buena parte de los sistemas de localización en tiempo real (RTLS, del inglés \textit{real-time locating system}) que se despliegan hoy en logística, control de acceso y llave digital vehicular. Esa precisión se degrada de manera significativa cuando el trayecto directo entre los dos dispositivos se encuentra obstruido, condición conocida como NLoS. En esa situación el receptor identifica como primer camino una réplica reflejada de la señal y la distancia estimada resulta mayor que la real, con errores que pueden alcanzar varios decímetros. La detección confiable de la condición NLoS es, por lo tanto, el mecanismo principal disponible para mejorar la exactitud del \textit{ranging}.

En los productos actuales esa detección se resuelve mediante algoritmos de procesamiento digital de señales integrados en el propio chip, que aplican reglas deterministas sobre un conjunto reducido de métricas de canal, como la amplitud del primer camino o la potencia total recibida. El enfoque tiene la ventaja del determinismo y del bajo consumo, pero descarta la mayor parte de la información contenida en la respuesta al impulso del canal. En paralelo, la literatura reciente muestra que los modelos de aprendizaje profundo aplicados sobre la CIR completa alcanzan tasas de acierto muy superiores. Esos trabajos se ejecutan fuera del chip, sobre un procesador de propósito general, porque su tamaño previsiblemente excede la memoria disponible en un transceptor UWB. Un ejemplo representativo es la arquitectura ADCNN-BiLSTM-Transformer propuesta por Qi et al.\footnote{L.~Qi, Y.~Zhao, Q.~Guo, M.~Kaliuzhnyi y E.~Wang, ``Deep learning-based NLOS identification of UWB signals'', \textit{IEEE Signal Process. Lett.}, vol.~32, pp.~2394--2398, 2025, doi: \href{https://doi.org/10.1109/LSP.2025.3577127}{10.1109/LSP.2025.3577127}.}, que combina convoluciones dilatadas adaptativas, una red recurrente bidireccional de memoria de corto y largo plazo y un mecanismo de atención de tipo \textit{transformer}.

El proyecto se ubica en ese espacio intermedio. Se propone desarrollar una red neuronal convolucional (CNN, del inglés \textit{convolutional neural network}) unidimensional de bajo perfil en memoria, cuantizada a enteros de 8 bits, que clasifique la CIR como LoS o NLoS con una exactitud comparable a la de los modelos publicados, pero con un tamaño compatible con la memoria de un dispositivo embebido. El entregable es un producto mínimo viable que demuestra la factibilidad de trasladar la inferencia al borde, como paso previo a su integración en un transceptor comercial de la familia NXP SR250 o NCJ29D6.

El contexto es empresarial. El trabajo se realiza en MobileKnowledge, compañía especializada en tecnologías de conectividad de corto alcance y socia tecnológica de NXP Semiconductors. La empresa aporta el conocimiento de dominio en UWB y el acceso al hardware de evaluación. El proyecto no cuenta con financiamiento público ni privado específico. La documentación técnica, el código de ejemplo y las especificaciones de los transceptores de NXP que se emplean son de acceso público, de modo que ninguna restricción de confidencialidad recae sobre el material técnico del proyecto. La propiedad intelectual del modelo que se desarrolle corresponde a MobileKnowledge, por lo que la difusión de los resultados se acuerda con la empresa.

La propuesta de valor se apoya en tres puntos. Primero, la solución opera sobre datos que el transceptor ya calcula, sin necesidad de sensores adicionales ni de cambios en el protocolo de \textit{ranging}. Segundo, al ejecutarse en el dispositivo, elimina la latencia y el consumo que implica enviar a un procesador externo la CIR completa, que por su longitud representa un volumen de datos considerable para un enlace embebido. Tercero, el modelo se diseña desde el inicio con un presupuesto de memoria explícito, criterio ausente en los trabajos académicos de referencia, que priorizan la exactitud sin restricciones de plataforma.
...
\node[target, right=0.4cm of b3] (b4) {Motor de inferencia\\LiteRT\\y CMSIS-NN};
...
En términos de ciclo de vida, la solución se encuentra en etapa de prueba de concepto. El proyecto la lleva hasta un prototipo validado sobre hardware de evaluación, con métricas de exactitud, memoria y latencia medidas. La integración en un producto comercial queda fuera del alcance y depende de decisiones posteriores de la empresa.
...
\begin{itemize}
    \item Cliente: Carlos Paternain aprueba los entregables y participa en la definición de los criterios de aceptación. Conoce en profundidad la tecnología UWB y su mercado, por lo que su revisión es determinante para validar que la planificación sea realista.
...
El propósito de este proyecto es demostrar que la detección de la condición NLoS en enlaces UWB puede resolverse con un modelo de aprendizaje profundo lo suficientemente pequeño como para ejecutarse dentro de un dispositivo embebido, y no solo en un procesador externo, con el fin de mejorar la exactitud de la medición de distancia sin agregar hardware, sin modificar el protocolo de \textit{ranging} y sin transferir la respuesta al impulso del canal fuera del dispositivo.
...
\begin{itemize}
    \item La construcción de un flujo reproducible de ingesta del conjunto de datos público eWINE\footnote{Conjunto de datos \texttt{UWB-LOS-NLOS-Data-Set}, SensorLab, Jožef Stefan Institute, 2017, bajo licencia CC BY 4.0. Disponible en \href{https://github.com/ewine-project/UWB-LOS-NLOS-Data-Set}{github.com/ewine-project/UWB-LOS-NLOS-Data-Set}. Los autores solicitan citar: K.~Bregar, A.~Hrovat y M.~Mohorčič, ``NLOS channel detection with multilayer perceptron in low-rate personal area networks for indoor localization accuracy improvement'', en \textit{Proc. 8th Jožef Stefan International Postgraduate School Students' Conference}, Liubliana, Eslovenia, 2016.}, con particiones versionadas para entrenamiento, validación y prueba.
    ...
    \item La cuantización del modelo a enteros de 8 bits, la verificación de que sus operadores estén implementados en LiteRT for Microcontrollers, denominado TensorFlow Lite for Microcontrollers hasta su cambio de marca, y la comprobación de que los de mayor cómputo cuenten con núcleo optimizado en CMSIS-NN.
    \item El desarrollo del firmware de prueba que ejecuta la inferencia sobre un SoC basado en ARM Cortex-M33, con aceleración mediante CMSIS-NN (Common Microcontroller Software Interface Standard, Neural Networks), la biblioteca de núcleos de redes neuronales que ARM optimiza para esos procesadores.
    ...
    \item La documentación técnica interna para MobileKnowledge, que comprende la ficha técnica del modelo y la guía de integración en el firmware.
...
\begin{itemize}
    ...
    \item La integración del modelo dentro del firmware de producción de los transceptores SR250 o NCJ29D6.
...
\begin{itemize}
    ...
    \item La empresa provee la placa de evaluación con un SoC basado en ARM Cortex-M33 y su cadena de compilación durante toda la etapa de despliegue.
    \item NXP Semiconductors mantiene disponibles, sin cambios incompatibles, el SDK, las herramientas de desarrollo y la documentación de los transceptores SR250 y NCJ29D6 durante la etapa de despliegue.
    ...
    \item Las herramientas de código abierto empleadas, entre ellas TensorFlow, LiteRT for Microcontrollers y CMSIS-NN, conservan sus licencias permisivas actuales.
...
El Product Backlog se organiza en cuatro épicas que se corresponden con los bloques funcionales del sistema descrito en la sección \ref{sec:descripcion}: la obtención y comprensión de los datos, su preparación, el modelado y su optimización, y el despliegue sobre el sistema embebido. Las actividades de documentación, el informe de avance, la memoria técnica y la defensa no forman parte del backlog: se planifican como tareas de los sprints en la sección~\ref{sec:sprints}.
...
\textbf{Criterio de prioridad.} Cada historia de usuario recibe una de tres prioridades:

\begin{itemize}
    \item Alta: sin ella el prototipo comprometido en el acta de constitución no puede construirse o no puede verificarse, o bien sostiene un plan de mitigación de la sección~\ref{sec:riesgos}.
    \item Media: mejora la calidad o la solidez del resultado y, ante un atraso, admite reducir su alcance sin comprometer el entregable.
    \item Baja: es opcional y se ejecuta solo si el cronograma lo permite.
\end{itemize}

El orden de ejecución no sigue estrictamente la prioridad, porque dos historias de prioridad media producen insumos que las siguientes necesitan: HU2 define el esquema de partición con el que se evalúan los modelos, y HU4 fija el ancho de ventana con el que se entrenan. En ambos casos, reducir el alcance significa acotar el análisis, no omitir el insumo.
...
\begin{itemize}
    ...
    \item \textbf{HU2.} Como desarrollador de modelos, quiero un análisis exploratorio de las CIR discriminado por clase y por entorno, para identificar sesgos de distribución y definir un esquema de partición que permita medir la generalización. Prioridad media.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{HU4.} Como desarrollador de modelos, quiero reducir la ventana de CIR al menor número de muestras que preserve la exactitud, para acotar la memoria de trabajo que el modelo requiere en el dispositivo. Prioridad media.
\end{itemize}
...
\begin{itemize}
    \item \textbf{HU8.} Como desarrollador de firmware, quiero ejecutar el modelo cuantizado sobre el SoC basado en ARM Cortex-M33, para verificar que la inferencia entra en la memoria disponible y responde dentro del tiempo requerido. Prioridad alta.
    ...
    \item \textbf{HU10.} Como responsable técnico del producto, quiero comparar el detector basado en aprendizaje profundo contra el criterio determinista de referencia, para cuantificar la mejora que aporta la solución propuesta. Prioridad baja. Historia opcional.
\end{itemize}
...
HU2 & Ingesta y comprensión de los datos & 2 & 1 & 1 & 4 & 5 & Media \\ \hline
HU3 & Preprocesamiento y representación & 3 & 3 & 2 & 8 & 8 & Alta \\ \hline
HU4 & Preprocesamiento y representación & 3 & 2 & 2 & 7 & 8 & Media \\ \hline
...
HU10 & Despliegue y validación & 2 & 3 & 3 & 8 & 8 & Baja (opcional) \\ \hline
\rowcolor[HTML]{EFEFEF}
\multicolumn{6}{|r|}{Total} & 89 & \\ \hline
\end{tabularx}
\end{table}
\else\input{plantilla/06-product-backlog}\fi
...
\begin{itemize}
    \item El flujo procesa la totalidad de los registros del conjunto de datos y finaliza en menos de 5 minutos sobre la computadora de desarrollo.
...
\begin{itemize}
    ...
    \item Cada CIR normalizada presenta energía unitaria, lo cual se verifica con una tolerancia relativa de $10^{-6}$.
...
\begin{itemize}
    \item La red convolucional alcanza un valor de F1 igual o superior a 0,88 sobre el conjunto de prueba con partición estratificada.
    \item Se reportan exactitud, precisión, exhaustividad, F1, área bajo la curva ROC (del inglés \textit{receiver operating characteristic}) y matriz de confusión.
    \item La red supera en F1 a los dos modelos de comparación entrenados sobre las métricas de canal.
    \item Los resultados se reproducen a partir del registro de experimentos, que conserva la configuración, la semilla y los pesos de cada ejecución.
\end{itemize}
...
\begin{itemize}
    ...
    \item La totalidad de los operadores del grafo cuantizado pertenece al conjunto que implementa LiteRT for Microcontrollers, condición sin la cual el modelo no se ejecuta en el dispositivo. Se verifica con la herramienta de inspección del grafo.
    \item Las operaciones que concentran el cómputo, convoluciones y capas densas, se resuelven con los núcleos optimizados de CMSIS-NN y no con las implementaciones de referencia, lo que se verifica con el perfilador de operadores del intérprete.
...
\end{itemize}
\else\input{plantilla/07-criterios-aceptacion}\fi
...
El objetivo es aumentar la exactitud de la medición de distancia en enlaces UWB mediante la identificación confiable de la condición NLoS, resuelta con un modelo de aprendizaje profundo que se ejecuta dentro del dispositivo. El impacto esperado es doble: por un lado, la reducción del error de \textit{ranging} en escenarios de interior con obstrucciones, que son los más frecuentes en aplicaciones de logística y control de acceso; por el otro, la posibilidad de descartar mediciones poco confiables antes de que contaminen el cálculo de posición.

Se adoptan tres métricas de éxito conjuntas, que deben cumplirse de manera simultánea:

\begin{itemize}
    \item Un valor de F1 igual o superior a 0,85 para el modelo desplegado, en la clasificación entre LoS y NLoS sobre el conjunto de prueba.
    \item Una ocupación de memoria de programa igual o menor a 64 kB para el modelo cuantizado.
    \item Una latencia de inferencia mediana igual o menor a 10 ms sobre el SoC basado en ARM Cortex-M33.
\end{itemize}

El umbral de F1 se fija en relación con el estado del arte. Qi et al.\footnote{Véase la referencia citada en la nota 1 (Qi et al., 2025).} reportan una exactitud de 92,19~\% sobre el mismo conjunto de datos, con un modelo que combina capas convolucionales, una capa recurrente y un mecanismo de atención, concebido para ejecutarse fuera del chip. El presente proyecto persigue un modelo restringido a 64 kB y cuantizado a enteros de 8 bits, condiciones que previsiblemente implican una pérdida de exactitud. Por ese motivo el objetivo no es igualar ese resultado, sino acotar la brecha: un F1 de 0,85 equivale a una diferencia de alrededor de siete puntos porcentuales, que es el costo que se acepta a cambio de habilitar la inferencia dentro del dispositivo. Como el conjunto de datos está balanceado entre clases, F1 y exactitud toman valores próximos, lo que vuelve razonable la comparación.

El umbral se traslada a los criterios de aceptación de manera encadenada. La poda admite una pérdida de hasta 1 punto porcentual de F1 (HU6) y la cuantización hasta 2 puntos (HU7), de modo que la red de referencia debe alcanzar un F1 de al menos 0,88 (HU5) para que el modelo desplegado conserve el 0,85 aun cuando ambas pérdidas se produzcan.

\subsection{Comprensión de los datos}

Se trabaja con datos tabulares que contienen una señal unidimensional por registro. La fuente es el conjunto de datos público del proyecto eWINE, que reúne trazas capturadas con transceptores UWB en siete ubicaciones de interior distintas. El conjunto reúne 42.000 registros balanceados entre las dos clases, con 3.000 muestras de cada condición en cada ubicación, capturados con dos nodos UWB, uno como ancla y otro como etiqueta, sobre un módulo de radio DWM1000 de DecaWave. El repositorio aclara que solo se registraron trazas de canal, sin posicionamiento de referencia, por lo que el conjunto no sirve para evaluar localización.

Cada registro reúne dieciséis campos: la respuesta al impulso del canal muestreada en 1.016 puntos y un conjunto de métricas que el propio transceptor calcula, entre ellas el índice del primer camino (\texttt{FP\_IDX}), las amplitudes de los tres primeros picos (\texttt{FP\_AMP1}, \texttt{FP\_AMP2} y \texttt{FP\_AMP3}), la potencia total del canal (\texttt{CIR\_PWR}) y el número de símbolos acumulados durante el preámbulo (\texttt{RXPACC}).

Respecto de la calidad, el conjunto se encuentra balanceado y no presenta valores faltantes, lo que reduce el trabajo de imputación. Existen, en cambio, dos limitaciones relevantes que condicionan el proyecto. La primera es que la totalidad de las trazas proviene de un único modelo de transceptor, de modo que las características del receptor quedan incorporadas en los datos. La segunda es que los siete entornos corresponden a un mismo tipo de construcción, lo que restringe la variedad de condiciones de propagación representadas. Ambas limitaciones se tratan en la sección \ref{sec:gobernanza} y en la sección \ref{sec:riesgos}.
...
El despliegue es local y se realiza dentro del dispositivo, sin ningún componente en la nube. El modelo cuantizado se convierte en un arreglo de constantes en C que se compila y se enlaza junto con el firmware, de manera que queda alojado en la memoria de programa del dispositivo. El intérprete de LiteRT for Microcontrollers lo lee desde allí y lo ejecuta apoyado en los núcleos optimizados de CMSIS-NN. La imagen resultante se graba en la placa de evaluación con ARM Cortex-M33. El banco de pruebas inyecta CIR almacenadas en memoria no volátil y entrega el resultado de la inferencia por el puerto serie, lo que permite automatizar la campaña de medición desde la computadora de desarrollo.
\else\input{plantilla/08-crisp-dm}\fi

\ifnum\revision>1
\section{9. Desglose del trabajo en tareas}
\label{sec:wbs}

En la tabla \ref{tab:tareas} se descompone cada historia de usuario en tareas técnicas concretas, con su estimación horaria y su prioridad relativa. La prioridad de una tarea se asigna según su impacto en los criterios de aceptación y no supera la de la historia a la que pertenece. La estimación considera la complejidad técnica, el nivel de incertidumbre y la experiencia previa del autor, e incluye el tiempo de prueba y de documentación de cada tarea.
...
HU2 & Calcular los estadísticos descriptivos de las métricas de canal por clase y por entorno & 5 h & Media \\ \hline
...
HU4 & Parametrizar el ancho de la ventana de CIR alrededor del primer camino & 4 h & Media \\ \hline
HU4 & Ejecutar el barrido de anchos de ventana entre 32 y 256 muestras y registrar exactitud y memoria & 8 h & Media \\ \hline
HU4 & Seleccionar el ancho de ventana y documentar el compromiso entre exactitud y memoria & 3 h & Media \\ \hline
...
HU7 & Verificar que todos los operadores del grafo estén implementados en LiteRT for Microcontrollers y que los de mayor cómputo tengan núcleo en CMSIS-NN & 5 h & Alta \\ \hline
...
HU8 & Configurar el proyecto de firmware con LiteRT for Microcontrollers y CMSIS-NN para el ARM Cortex-M33 & 8 h & Alta \\ \hline
...
HU10 & Extraer el indicador determinista de NLoS de referencia para el conjunto de prueba & 6 h & Baja \\ \hline
HU10 & Construir la comparación de matrices de confusión y curvas ROC entre el criterio determinista y el modelo & 6 h & Baja \\ \hline
\rowcolor[HTML]{EFEFEF}
\multicolumn{2}{|r|}{Total} & 174 h & \\ \hline
\end{longtable}
\else\input{plantilla/09-desglose-tareas}\fi

\ifnum\revision>2
\section{10. Planificación de Sprints}
\label{sec:sprints}

El proyecto se organiza en doce sprints. El Sprint 0 acompaña la cursada de Gestión de Proyectos y se dedica a la planificación. Los sprints 1 a 10 son técnicos, tienen una duración de tres semanas cada uno y una carga de 48 horas. El Sprint 11 ocupa, de manera estimada, los dos últimos bimestres del posgrado y comprende las actividades de los talleres de Trabajo Final A y B (TTFA y TTFB): en el primero se redactan los tres primeros capítulos de la memoria técnica, y en el segundo, los dos restantes junto con la preparación de la defensa. El Sprint 0 y el Sprint 11 exceden la duración de tres semanas de los sprints técnicos porque siguen el calendario de las asignaturas del posgrado. A esos sprints se suman dos actividades transversales que atraviesan todo el proyecto, entre ellas el informe de avance, que se entrega antes del TTFA.

La documentación técnica interna para MobileKnowledge se produce de manera incremental: cada sprint técnico incluye una tarea que documenta lo realizado, y los sprints 9 y 10 cierran con la guía de integración del modelo en el firmware y con la ficha técnica del modelo. Esas tareas nutren a su vez los capítulos de diseño y de resultados de la memoria.

\textbf{Jornada de trabajo adoptada.} Durante los sprints activos se planifican 16 horas semanales, distribuidas en 3 horas de lunes a viernes y 1 hora los sábados. El cronograma incorpora un receso de cuatro semanas entre diciembre y enero, y un período de contingencia de dos semanas entre el cierre técnico y el inicio del TTFA. A esa holgura se suma la baja carga del Sprint 11, de alrededor de cuatro horas semanales, que durante la redacción de los capítulos introductorios deja capacidad para absorber desvíos del desarrollo técnico. Sobre el calendario completo de 64 semanas, la dedicación promedio resulta de unas 9,4 horas semanales.
...
Sprint 8 & HU7 & Verificar los operadores del grafo en LiteRT y su cobertura en CMSIS-NN & 5 h & Alumno & 0 \% \\ \hline
...
Sprint 9 & HU8 & Configurar la cadena de compilación y de depuración, y archivar la versión fijada del SDK con su documentación & 8 h & Alumno & 0 \% \\ \hline
...
Sprint 9 & HU8 & Redactar la guía interna de integración del modelo en el firmware & 4 h & Alumno & 0 \% \\ \hline
...
Sprint 10 & HU9 & Elaborar la ficha técnica del modelo con los resultados del prototipo & 5 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFA & Redactar el capítulo 1 de la memoria, introducción general & 8 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFA & Redactar el capítulo 2 de la memoria, introducción específica & 10 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFA & Redactar el capítulo 3 de la memoria, diseño e implementación & 14 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFB & Redactar el capítulo 4 de la memoria, ensayos y resultados & 12 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFB & Redactar el capítulo 5 de la memoria, conclusiones & 5 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFB & Revisar, corregir y maquetar la versión final de la memoria & 6 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFB & Preparar la presentación de la defensa & 9 h & Alumno & 0 \% \\ \hline
Sprint 11 & TTFB & Ensayar la defensa y ajustar los tiempos de exposición & 6 h & Alumno & 0 \% \\ \hline
...
Receso programado & - & 4 & 0 h \\ \hline
Contingencia & - & 2 & 0 h \\ \hline
Memoria técnica y defensa (TTFA y TTFB) & Sprint 11 & 18 & 70 h \\ \hline
...
\ganttbar[bar/.append style={fill=colorPausa}]{Contingencia}{45}{46} \\
\ganttbar[bar/.append style={fill=colorNoTecnico}]{S11. TTFA}{47}{55} \\
\ganttbar[bar/.append style={fill=colorNoTecnico}]{S11. TTFB}{56}{64} \\
...
El cronograma cubre la totalidad de las 600 horas planificadas y finaliza en la fecha de la defensa pública. Los desvíos de los sprints técnicos se absorben en el período de contingencia de dos semanas y en la capacidad libre que deja la baja carga del TTFA. La etapa de despliegue es la que concentra la mayor incertidumbre, y es también la inmediatamente anterior a ese margen. El TTFB, que coincide con el quinto bimestre, queda reservado a la finalización de la memoria técnica y a la preparación de la defensa.
\else\input{plantilla/11-diagrama-gantt}\fi
...
\textbf{Normativa de protección de datos personales.} Los datos que se emplean son respuestas al impulso del canal radioeléctrico y métricas calculadas por el receptor. Se trata de mediciones físicas de un enlace de comunicaciones que no contienen identificadores de personas, ni permiten por sí mismas identificar a una persona humana determinada o determinable. Por ese motivo no resultan aplicables la Ley 25.326 de Protección de Datos Personales de la República Argentina ni el Reglamento General de Protección de Datos de la Unión Europea. El repositorio del conjunto de datos eWINE documenta que cada medición se realizó entre dos nodos UWB y que solo se registraron trazas de canal, sin posicionamiento de referencia. No especifica cómo se generó la obstrucción en cada captura: la conclusión se apoya en que los datos registrados no incluyen identificadores de ningún tipo.

Corresponde señalar que esa conclusión vale para el conjunto de datos y para el alcance del presente proyecto. Un despliegue posterior de la solución dentro de un sistema de localización que siga la posición de personas identificables sí quedaría comprendido en ambas normas, dado que los datos de localización constituyen datos personales. En ese escenario serían exigibles la base legal para el tratamiento, la información al titular de los datos y las medidas de seguridad correspondientes. La advertencia se deja asentada para una eventual continuación del trabajo con fines comerciales.

\textbf{Licenciamiento del conjunto de datos.} El repositorio del conjunto de datos eWINE declara la titularidad de SensorLab, del Jožef Stefan Institute, y lo distribuye bajo licencia Creative Commons Attribution 4.0 International (CC BY 4.0). Esa licencia permite copiar, modificar y redistribuir los datos, incluso con fines comerciales, con la condición de atribuir la autoría, enlazar la licencia e indicar si se introdujeron cambios. Los autores solicitan además citar el trabajo científico asociado a su publicación, que se referencia en la sección \ref{sec:alcance}. El proyecto cumple ambas condiciones en los entregables y en la memoria técnica.

El punto tiene consecuencia práctica: como la licencia admite el uso comercial, no impone restricciones a la explotación posterior del modelo por parte de MobileKnowledge, siempre que se conserve la atribución del conjunto de datos de origen.

\textbf{Licenciamiento de las herramientas y de los modelos.} Las bibliotecas empleadas para el entrenamiento y la inferencia, entre ellas TensorFlow, LiteRT for Microcontrollers y CMSIS-NN, se distribuyen bajo licencia Apache 2.0, que permite el uso, la modificación y la distribución, incluso con fines comerciales, con la condición de conservar los avisos de copyright y de licencia. El proyecto no emplea modelos preentrenados de terceros: la totalidad de los pesos se obtiene por entrenamiento propio sobre el conjunto de datos citado.

La documentación técnica, las notas de aplicación y el código de ejemplo de los transceptores SR250 y NCJ29D6 de NXP Semiconductors son de acceso público, de manera que pueden citarse y emplearse en los entregables sin restricciones de confidencialidad. El autor mantiene, en su condición de empleado de MobileKnowledge, acuerdos de confidencialidad con la empresa y con NXP que alcanzan a otros proyectos, pero no al material técnico utilizado en este trabajo.

La propiedad intelectual del modelo desarrollado corresponde a MobileKnowledge. En consecuencia, su publicación, su licenciamiento y cualquier uso posterior quedan sujetos a la autorización de la empresa, y el alcance de lo que se difunde en la memoria técnica se acuerda con el cliente antes de la entrega.
...
\else\input{plantilla/12-gobernanza-datos}\fi
...
\textbf{Riesgo 3:} no se dispone de la placa de evaluación al comienzo de la etapa de despliegue, por demoras en el aprovisionamiento.
\begin{itemize}
    ...
    \item Probabilidad de ocurrencia (O): 2. La empresa provee el hardware y es socio de nivel gold del fabricante, condición que le da un canal de aprovisionamiento directo. El antecedente disponible es un pedido recibido dentro de las dos semanas.
\end{itemize}
...
\begin{itemize}
    \item Severidad (S): 6. Se pierde la instancia de validación de los entregables y el acceso al conocimiento de dominio, lo que degrada la calidad de las decisiones técnicas respecto UWB, al quedar sin revisión experta, sin impedir la ejecución del proyecto.
...
\textbf{Riesgo 8:} NXP Semiconductors discontinúa o modifica de manera incompatible el SDK, las herramientas de desarrollo o la documentación de los transceptores SR250 y NCJ29D6 durante la etapa de despliegue.
\begin{itemize}
    \item Severidad (S): 5. Un cambio incompatible en esa etapa obligaría a adaptar el proyecto de firmware a la nueva versión, con reprocesamiento de parte de la integración y un posible atraso del prototipo. No afectaría al modelo ni al trabajo de los sprints 1 a 8, que se ejecuta en la computadora de desarrollo sin herramientas del fabricante.
    \item Probabilidad de ocurrencia (O): 2. La exposición se limita a los sprints 9 y 10: antes de esa etapa el proyecto no usa herramientas del fabricante y puede adoptar la versión vigente al comenzarla. El SDK se distribuye por versiones, de modo que una actualización no invalida por sí misma la versión en uso, y la familia de productos se encuentra vigente y con demanda comercial activa, lo que hace improbable su discontinuación en ese plazo.
\end{itemize}

\textbf{b) Tabla de gestión de riesgos}

El número de prioridad del riesgo (RPN, del inglés \textit{risk priority number}) se calcula como el producto entre severidad y ocurrencia, RPN $=$ S $\times$ O. En la tabla \ref{tab:riesgos} se presentan los valores originales y los valores posteriores a la mitigación, identificados con asterisco.
...
R1. El modelo no se transfiere al transceptor de destino & 8 & 6 & 48 & 6 & 3 & 18 \\ \hline
R2. El modelo cuantizado excede la memoria disponible & 7 & 5 & 35 & 5 & 2 & 10 \\ \hline
R3. Falta de disponibilidad de la placa de evaluación & 9 & 2 & 18 & & & \\ \hline
R4. Degradación excesiva de la exactitud por cuantización & 6 & 5 & 30 & & & \\ \hline
R5. Reducción sostenida de la disponibilidad horaria & 7 & 7 & 49 & 6 & 4 & 24 \\ \hline
R6. Cambio de prioridades comerciales de la empresa & 6 & 4 & 24 & & & \\ \hline
R7. Indisponibilidad del conjunto de datos público & 5 & 2 & 10 & & & \\ \hline
R8. Cambio incompatible en el soporte del fabricante & 5 & 2 & 10 & & & \\ \hline
\end{tabularx}%
\end{table}

Criterio adoptado: se elaboran planes de mitigación para los riesgos cuyo RPN resulta mayor que 30. Los riesgos R3, R6, R7 y R8 quedan por debajo de ese umbral y R4 se ubica exactamente en él. Ninguno lo supera, por lo que se someten a seguimiento sin acción de mitigación específica.

Nota: los valores marcados con asterisco corresponden a la situación posterior a la aplicación de la mitigación. Las filas que no los presentan corresponden a riesgos que, por mo superar el umbral, no requieren un plan de mitigación.
...
\textbf{Riesgo 5.} Se adopta una jornada fija y acotada de 16 horas semanales durante los sprints activos, valor sostenible en paralelo con la actividad profesional. El cronograma incorpora un receso programado de cuatro semanas y un período de contingencia de dos semanas previo al TTFA, al que se suma la capacidad libre que deja la baja carga de redacción durante los capítulos introductorios de la memoria. La historia de usuario HU10 se declara opcional y las historias HU2 y HU4, de prioridad media, admiten reducir su alcance, de manera que ante un atraso se libera carga sin afectar el entregable comprometido. El avance se controla al cierre de cada sprint mediante el porcentaje de completitud de la tabla \ref{tab:sprints}.
\begin{itemize}
    \item Severidad (S*): 6. La contingencia exclusiva es de dos semanas, pero la capacidad libre del TTFA, una historia opcional y dos de prioridad media que admiten reducir su alcance permiten absorber un desvío de magnitud moderada sin desplazar la fecha de la defensa. Un desvío mayor comprometería la redacción del capítulo de diseño e implementación, que depende de que el desarrollo técnico esté cerrado.
...
\multirow{-2}{=}{HU5 \newline Sprint 5} & Evaluar con exclusión de un entorno completo & \multirow{-2}{=}{Modelo de línea base con su informe de métricas} & \multirow{-2}{=}{F1 igual o superior a 0,88 en el conjunto de prueba y superioridad frente a los modelos de comparación} & \multirow{-2}{=}{El desempeño con exclusión de entorno puede resultar sensiblemente menor} \\
\hline
                      & Aplicar la cuantización a enteros de 8 bits & & & \\ \cline{2-2}
\multirow{-2}{=}{HU7 \newline Sprint 8} & Verificar los operadores y su cobertura en CMSIS-NN & \multirow{-2}{=}{Modelo cuantizado y su informe de verificación} & \multirow{-2}{=}{El modelo ocupa 64 kB o menos, pierde menos de 2 puntos de F1, sus operadores están implementados y los de mayor cómputo usan CMSIS-NN} & \multirow{-2}{=}{Riesgo R4: la degradación puede exigir entrenamiento consciente de la cuantización} \\
\hline
...
\end{tabular}
\end{table}
\else\input{plantilla/14-sprint-review}\fi
...
\end{tabular}
\end{table}
\else\input{plantilla/15-sprint-retrospective}\fi
